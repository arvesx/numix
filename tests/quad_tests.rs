...
mod general_test {
    ...
    fn test_quad_finite() {
        //done
        let results = Quad::initialize(|x| x.exp() + x.ln(), 2.0, 3.0).run();
    ...
    fn test_quad_finite2() {
        ...
        let now = Instant::now();
        let results =
            Quad::initialize(|x| x.powi(3) - 2.0 * x.powi(2) + x.sin() - 1.0, -1.0, 4.0).run();
    ...
    fn test_quad_conv_singularity_conv() {
        ...
        let now = Instant::now();
        let results = Quad::initialize(|x| 1.0 / (3.0 - x).sqrt(), 0.0, 3.0)
    ...
    fn test_quad_singularity2_div() {
        let results = Quad::initialize(|x| 1.0 / (3.0 - x).sqrt(), f64::NEG_INFINITY, 3.0)
    ...
    fn test_quad_posinfinite() {
        ...
        //done
        let now = Instant::now();
        let results = Quad::initialize(|x| (-x).exp() * x.sin(), 0.0, f64::INFINITY).run();
    ...
    fn test_quad_posinfinite2() {
        //done
        let results = Quad::initialize(|x| (1.0 / x.powi(2)), 1.0, f64::INFINITY).run();
    ...
    fn test_quad_posinfinite3_div() {
        let results = Quad::initialize(|x| (1.0 / x.powi(2)), 0.0, f64::INFINITY)
            .change_tolerance(1e-14)
            .run();
    ...
    fn test_quad_posinfinite4_div() {
        let results = Quad::initialize(|x| x.sin(), -2.0, f64::INFINITY)
            .change_tolerance(1e-14)
            .run();
    ...
    fn test_quad_neginfinite() {
        ...
        //done
        let now = Instant::now();
        let results = Quad::initialize(|x| (x).exp() * x.sin(), f64::NEG_INFINITY, 2.0).run();
        let elapsed = now.elapsed();
        println!("Elapsed: {:.2?}", elapsed);
        match results {
            Ok(results) => {
    ...
    fn test_quad_bilateralinfinite() {
        let results =
            Quad::initialize(|x| x * (-x * x).exp(), f64::NEG_INFINITY, f64::INFINITY).run();
