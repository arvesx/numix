...
mod general_test{
    ...
    fn test_bessel() {
        ...
        println!("{:?}", coef1);
        assert!(precision_equals_vectors(&coef1,
            &vec![1.0, 6.0, 15.0, 15.0],1e-8,0.0));

        println!("{:?}", coef2);
        assert!(precision_equals_vectors(&coef2,
            &vec![1.0, 10.0, 45.0, 105.0, 105.0],1e-8,0.0));

        println!("{:?}", coef3);
        assert!(precision_equals_vectors(&coef3,
            &vec![1.0, 15.0, 105.0, 420.0, 945.0, 945.0],1e-8,0.0));
    ...
    fn test_legendre() {
        ...
        println!("{:?}", coef1);
        assert!(precision_equals_vectors(&coef1,
            &vec![0.375,0.0,-3.750,0.0,4.375],1e-8,0.0));

        println!("{:?}", coef2);
        assert!(precision_equals_vectors(&coef2,
            &vec![0.0, 1.875, 0.0, -8.75, 0.0, 7.875],1e-8,0.0));

        println!("{:?}", coef3);
        assert!(precision_equals_vectors(&coef3,
            &vec![-0.3125, 0.0, 6.5625, 0.0, -19.6875, 0.0, 14.4375],1e-8,0.0));
    ...
    fn test_laguerre() {
        ...
        println!("{:?}", coef1);
        assert!(precision_equals_vectors(&coef1,
            &vec![1.0, -5.0, 5.0, -1.6666666666666667, 0.20833333333333334, -0.008333333333333333],1e-8,0.0));

        println!("{:?}", coef2);
        assert!(precision_equals_vectors(&coef2,
            &vec![1.0, -6.0, 7.5, -3.3333333333333335, 0.625, -0.05, 0.001388888888888889],1e-8,0.0));

        println!("{:?}", coef3);
        assert!(precision_equals_vectors(&coef3,
            &vec![1.0, -7.0, 10.5, -5.833333333333333, 1.4583333333333333, -0.175, 0.009722222222222222, -0.0001984126984126984],1e-8,0.0));
    }
    #[test]
    fn test_hermite() {
        //done
        ...
        println!("{:?}", coef1);
        assert!(precision_equals_vectors(&coef1,
            &vec![1680.0, 0.0, -13440.0, 0.0, 13440.0, 0.0, -3584.0, 0.0, 256.0],1e-8,0.0));

        println!("{:?}", coef2);
        assert!(precision_equals_vectors(&coef2,
            &vec![0.0, 30240.0, 0.0, -80640.0, 0.0, 48384.0, 0.0, -9216.0, 0.0, 512.0],1e-8,0.0));

        println!("{:?}", coef3);
        assert!(precision_equals_vectors(&coef3,
            &vec![-30240.0, 0.0, 302400.0, 0.0, -403200.0, 0.0, 161280.0, 0.0, -23040.0, 0.0, 1024.0],1e-8,0.0));
